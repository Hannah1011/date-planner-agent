...
# 구별 좌표로 API 호출. 구 이름이 없으면 서울 시청 좌표를 폴백으로 사용.
_SEOUL_LAT = 37.5665
_SEOUL_LON = 126.9780

_DISTRICT_COORDS: dict = {
...
def get_weather(district: str, date: str) -> dict:
    ...
    if coords:
        ...
        logger.warning("알 수 없는 구 이름: %s — 서울 중심 좌표로 폴백", district)
    ...
    try:
        ...
        logger.error("날씨 API 호출 실패 (%s, %s): %s", district, date, e)
        return {}
